	...
	<div class="content container-fluid">				
		...
		<div class="page-header">
			<div class="content-page-header">
				<h5><?php echo set_value('opponentLawyer'); ?></h5>
				<div class="list-btn">
					<ul class="filter-list">
						<li>
							<a class="btn btn-primary" onclick="add()" ><i class="fa fa-plus-circle me-2" aria-hidden="true"></i><?php echo set_value('createNewOponentLawyer'); ?></a>
						</li>
					</ul>
				</div>
			</div>
							...
							<table class="table table-center table-hover datatable" id='setData'>
								<thead class="thead-light">
									<tr>
										...
										<th><?php echo set_value('opponentLawyerPhone'); ?></th>
									</tr>
								</thead>
								<tbody > </tbody>
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								<label for="opponentLawyer" class="form-label"><?php echo set_value("opponentLawyer"); ?><span class="text-danger"> * </span></label>
...
$( document ).ready(function() {
	$('#opponentNationality').select2({
        dropdownParent: $('#opponentModal')
	});
})
*/
...
function add()
{
	$("#OLawyerForm")[0].reset();
	$('#id').val("0");
	$('#opponentNationality').val('').change();
	$('#layerModal').modal('toggle');
}
		...
		success: function (data) {
			...
			if(jsonObject.status)
			{
				...
				jQuery.each(data_array, function() {
					$('#id').val(this.id);
					$('#opponentLawyer').val(this.oppoLawyerName);
					$('#opponentLawyerPhone').val(this.oppoLawyerContact);
				});
				$('#layerModal').modal('toggle');
			}
			else 
