...
if (isset($_POST['lsMId'])) {
	$qry_getcontractdata = "SELECT c.`lsContractId`, c.`lsMasterId`,m.`ls_code`,`lsStageId`, s.lsStagesName_$language as lsStagesName,`amount`, `taxValue`, taxAmount, `totalAmount`, `contractDate`, `contractInvoiceNumber`, `contractEn`, `contractAr`, `contractFilePath`, remarks, c.`isActive` FROM `tbl_lawsuit_contract` c 
		LEFT JOIN `tbl_lawsuit_master` m ON m.`lsMasterId`=c.`lsMasterId`
		LEFT JOIN `tbl_lawsuit_stages` s ON s.`lsStagesId`=c.`lsStageId`
		WHERE c.`isActive`=1 AND c.`lsMasterId`=:lsMasterId AND c.`lsContractId`=:contractId";
	$stmt_getcontractdata = $dbo->prepare($qry_getcontractdata);
	$stmt_getcontractdata->bindParam(":lsMasterId", $_POST['lsMId'], PDO::PARAM_INT);
	$stmt_getcontractdata->bindParam(":contractId", $_POST['contractId'], PDO::PARAM_INT);
			...
			<div class="invoice-wrapper download_section">
				<div class="inv-content">
					<div class="invoice-header" style="padding: 20px;">
						<div class="invoice-title" style="color: #2c3038; font-size: 24px; font-weight: 700;"><?php echo set_value('contract_invoice'); ?></div>
					</div>
						...
						<div class="inv-header-right">
							...
							<div class="inv-details" style="display: block;">
								<div class="inv-date" style="margin-right: 0px;">
									<?php echo set_value('date'); ?>: <span><?php echo $result_contactdata[0]['contractDate'];; ?></span>
								</div>
								<div class="inv-date">
									<?php echo set_value('invoice_number'); ?>: <span><?php echo $result_contactdata[0]['contractInvoiceNumber']; ?></span>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="invoice-address">
						...
						<div class="company-details d-flex flex-wrap">
							...
							<div class="gst-details col-6 d-flex">
								<b><?php echo set_value('lawsuitId'); ?></b>: <span>&nbsp;<?php if (empty($resultLawsuitDetails[0]['lawsuitId'])) echo "-";
																							else echo $resultLawsuitDetails[0]['lawsuitId']; ?></span>
							</div>
							<div class="gst-details col-6 d-flex">
							...
							<div class="gst-details col-3 d-flex">
								<b>Subject</b>: <span>&nbsp;<?php if (!empty($resultLawsuitDetails[0]['lsSubject'])) echo $resultLawsuitDetails[0]['lsSubject'];
															else echo '-'; ?></span>
				    		</div> -->
							<div class="gst-details col-6 d-flex">
							...
							<div class="gst-details col-6 mb-0 d-flex">
								<b><?php echo set_value('opponent'); ?></b>:<span>&nbsp;<?php echo trim($resultLawsuitDetails[0]['oppoName']); ?></span>
							</div>
							<div class="gst-details col-6 mb-0 d-flex">
