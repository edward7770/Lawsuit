<?php 
	...
	function set_file_name($file,$filename)
	{
		...
		$new_file_name = $new_file_name.date("YmdGis.").$ext;
	...
	function check_file($file)
	{
		...
		{
			$msg='0Please upload jpeg / jpg / png / gif / pdf / docx / xlsx file only';
			exit($msg);
		}
		...
		if(isset($filename) && $_FILES[$file]["error"] == 0){
			if($filesize > $maxsize)
			{
				$msg="0Error: File size of $filename is larger than the allowed limit.";
				exit($msg);
			}
		}
		else
		{
			echo "0Error: " . $_FILES["file"]["error"];
		}
		/*
	...
	function upload_file($file,$file_path,$filename)
	{
		...
		{
			$msg='0Failed to upload file. Please try again';
			exit($msg);
		}
	}
	...
	function deleteDirectory($postData)
	{
		foreach($postData as $i=> $row)
		{
			foreach($row as $index=>$val)
			{
				...
				if($index=='nationalAddressImage' && !empty($val))
				{
					unlink($val);
				}
				if($index=='idDefendantImage' && !empty($val))
				{
					unlink($val);
				}
			}
